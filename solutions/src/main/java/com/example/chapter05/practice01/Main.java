...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    for (String line : raw) {
      ...
      // (5) 空白除去後の結果が "N/A" のみのときは出力しない（equals）
      if (cleaned.equals("N/A")) { // 修正。文字列の内容比較のため、==（参照比較）から equals（内容比較）に修正。
        continue;
      }

      // (6) 2文字未満の行は出力しない（length）
      if (cleaned.length() >= 2) { // 修正。仕様「2文字以上のみ出力」のため、> 2 から >= 2 に修正。
        String label = "";

        // (7) 文字列に "JP" を含む場合（大文字小文字は問わない）、先頭に "[JP] " を付ける（contains）
        if (cleaned.contains("JP")) { // 修正。部分一致の仕様のため、equals から contains に修正。
          label = "[JP] ";
        }
  ...
  static String clean(String s) {
    // (1) 先頭末尾の空白を除去する（trim）
    s = s.trim(); // 追加。String は不変のため、戻り値を再代入して先頭末尾の空白を除去する。

    // (2) 全角スペース（\u3000） → 半角スペース ' ' に置換する（replace）
    s = s.replace('\u3000', ' '); // 追加。全角スペースを半角スペースに置換する。

    // (2) ハイフン '-' → 半角スペース ' ' に置換する（replace）
    s = s.replace('-', ' '); // 追加。ハイフン区切りを半角スペースに置換する。

    // (3) 空白1回以上の連続を分割する（split）
    String[] words = s.split("\\s+"); // 修正。空白1回以上にマッチさせる仕様のため、" " から "\\s+" に修正。

    // (3) 分割された文字列を半角スペース1つで結合して、1つの文字列にする。（join）
    s = String.join(" ", words); // 追加。分割された文字列を半角スペース1つを区切り文字として結合する。

    // (4) すべて大文字化する（toUpperCase）
    s = s.toUpperCase(); // 追加。アルファベットをすべて大文字化する。

    return s; // 修正。空文字の返却から、整形後の実際の結果を返すように修正。
  }
}